      ...
      <Tooltip title='Notifications' placement='bottom' arrow open={!open && tooltipShow}>
        ...
        >
          <Button
            ...
            size='large'
            color='text-primary'
            className={cn("icon-only hover-icon-shrink [&.active]:text-primary! hover:bg-grey-75", open && "active bg-grey-75")}
                ...
                <Card className='shadow-darker-sm! w-xs md:w-sm'>
                  ...
                  <TabContext value={tabValue}>
                    <TabList className='mb-5 px-4' onChange={handleTabValueChange}>
                      <Tab icon={<NiStructure size='tiny' />} iconPosition='start' label='All' className='tiny' value='1' />
                      <Tab icon={<NiScreen size='tiny' />} iconPosition='start' label='System' className='tiny' value='2' />
                      <Tab icon={<NiUsers size='tiny' />} iconPosition='start' label='User' className='tiny' value='3' />
                    </TabList>
...
function NotificationItem({
  ...
  const [anchorElEllipsis, setAnchorElEllipsis] = React.useState<EventTarget | Element | PopoverVirtualElement | null>(null);
          ...
          <Box className='flex flex-col items-start gap-2'>
            ...
            {chips && (
              <Box className='flex flex-row gap-1'>
                {chips.map((chip) => {
                  if (chip.image) {
                    return <Chip size='small' avatar={<Avatar alt='product' src={chip.image} />} label={chip.label} variant='outlined' key={chip.id} />;
                  } else {
